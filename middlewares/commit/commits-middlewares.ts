...
import { Commit_i, PopulatedCommit_i, PopulatedCommit2_i, PopulatedCommit3_i, PopulatedCommit4_i, PopulatedCommit5_i, TaskBase, PopulatedTaskBase, ContributorsCommitsMap, C_On_Repository, C_On_Layer } from '../../interfaces/interfaces';
import { Commit4Props, ContributorsUserData, Commit6Props, Commit7Props } from '../../types/types';


// ! Properties declared on the file express.d.ts

interface CommitsMiddlwaresRequest {
    hashes?: string[];
...
export const getContributorsCommits = async (req: Request, res: Response, next: NextFunction) => {
    const { taskId } = req.params;
    const { hashes, contributorsData = [] } = req;

    try {
        const commits: Commit4Props[] = await Commit.find({ uuid: { $in: hashes }})
        .select('uuid createdAt author associated_task')
        .sort({ createdAt: -1 })
        .lean();

        const initializedContributors = contributorsData.reduce((acc, contributor) => {
            const idKey = contributor?._id.toString()
            acc[idKey] = {
                id: contributor._id,
        ...
        commits.forEach(( commit: Commit4Props) => {
            //! const idKey = commit?.author?._id.toString();
            const idKey = commit?.author?.uid.toString();
...
export const getCommitsHashes = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {   
        if( uuid2 === '' && uuid1 !== ''){
            const commit1: PopulatedCommit2_i | null = await Commit.findOne({ uuid: uuid1 })
            ...
            const commit1: PopulatedCommit2_i | null = await Commit.findOne({ uuid: uuid2 })
                                .select('hash repository')
                                .populate('repository')
            ...
            const commit2: Pick<Commit_i, 'hash'> | null = await Commit.findOne({ uuid: uuid2 })
                                    .select('hash') as Pick<Commit_i, 'hash'> | null
...
export const findCommit = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const commit: Commit_i | null = await Commit.findOne({ uuid: hash });
...
export const getProjectCommitsBaseOnAccess = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        if( type === 'collaborator' ) {
            const commits: PopulatedCommit_i[] = await Commit.find(matchConditions)
            ...
            const filteredCommitsBaseOnLevel: PopulatedCommit3_i[] = ( await Promise.all( commits.map( async ( commit: PopulatedCommit_i ) => {
                ...
                if( cLayer && cRepo ) {               
                    ...
                    return commitWithIdsOnly as PopulatedCommit3_i;        
                }
            }))).filter( (commit): commit is PopulatedCommit3_i => commit !== undefined );

            
            // ! Commits en el caso de que el usuario no tiene acceso como colaborador ( state: false ), pero los padres son abiertos

            const uniqueCommitsOnOpenParents: PopulatedCommit3_i[] = ( await Promise.all( commits.filter( (openCommit: PopulatedCommit_i) => 
            ...
            const filteredCommitsBaseOnLevel = commits.reduce<PopulatedCommit3_i[]>((acc, commit) => {
                const { layer, repository } = commit;          
...
export const getCommitsDates = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const commits1: PopulatedCommit2_i[] = await Commit.find({ 'author.uid': uid, associated_task: null, createdAt: { $gte: startDate, $lte: endDate } })
                                    .select('createdAt uuid author _id repository branch')
                                    .populate('repository', 'name')
                                    .sort({ createdAt: -1 })
                                    .lean()

        const commits2: PopulatedCommit4_i[] = await Commit.find({ 'author.uid': uid, associated_task: { $ne: null }, createdAt: { $gte: startDate, $lte: endDate } })
...
export const getProjectCommitsDates = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const commits1: PopulatedCommit2_i[] = await Commit.find({ project: projectId, 'author.uid': uid, associated_task: null, createdAt: { $gte: startDate, $lte: endDate } })
                                    .select('createdAt uuid author _id repository branch')
                                    .populate('repository', 'name')
                                    .sort({ createdAt: -1 })
                                    .lean()

        const commits2: PopulatedCommit4_i[] = await Commit.find({ project: projectId, 'author.uid': uid, associated_task: { $ne: null }, createdAt: { $gte: startDate, $lte: endDate } })
                                    .select('createdAt uuid author _id repository branch associated_task')
                                    .populate('repository', 'name')
                                    .populate('associated_task', 'task_name')
                                    .sort({ createdAt: -1 })
...
export const getProfileCommitsFiltered = async(req: Request, res: Response, next: NextFunction) => {
    ...
    let matchCondition: MatchConditions2 = { 'author.uid': uid } ;
    if (year) {
        matchCondition = { 
        ...matchCondition,
        createdAt: {
    ...
    try {
        const commits: PopulatedCommit5_i[] = await Commit.find(matchCondition)
        ...
        const filteredCommits = commits.reduce<PopulatedCommit5_i[]>((acc, commit) => {
...
export const validateVisibility = (pVisisibility: string | undefined, lVisibility: string | undefined, rVisibility: string | undefined) => {
