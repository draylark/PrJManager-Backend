...
import { Layer_i, Collaborator_i, Repository_i, ExtendedLayer_i, GuestLayer_i, } from '../../interfaces/interfaces';
import { CombinedLayer_i } from '../../types/types';



// ! Properties declared on the file express.d.ts

interface layersMiddlwaresRequest {
    layer?: Layer_i;
    creatingMiddlewareState?: boolean;
    totalCreatedCollaborators?: number;
    layers?: CombinedLayer_i[] | GuestLayer_i[];
...
export const validateLayerExistance = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const layer: Layer_i | null = await Layer.findById(layerID);
...
export const updateOtherCDataOfLayerModifiedCollaborators = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const repos: Repository_i[] = projectRepos.length !== 0 
            ? projectRepos
...
export const updateOtherCDataOfDeletedLayerCollaborators = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        await Promise.all(deletedCollaborators.map( async ( id: string ) => {         
            const collaborators: Collaborator_i[] = await Collaborator.find({ uid: id, projectID, 'repository._id': { $exists: true } })
                                            .lean()
                                            .populate({
                                                path: 'repository._id',
            ...
            await Promise.all(collaborators.map(( collaborator: Collaborator_i ) => {
                ...
                const layer = (collaborator?.repository._id as Repository_i)?.layerID as Layer_i;
...
export const getProjectLayersDataBaseOnAccess = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        if( type === 'collaborator'){
            const collaboratorOnLayers: Collaborator_i[] = await Collaborator.find({ projectID, uid, state: true, 'layer._id': { $exists: true } })
                                                .populate('layer._id')
                                                .lean(); 

            // ! Layers on which the user is a collaborator
            const layersBaseOnLevel: ExtendedLayer_i[] = collaboratorOnLayers.map((collaborator: Collaborator_i): ExtendedLayer_i | undefined => {
                if (!collaborator.layer || !collaborator.layer._id || typeof collaborator.layer._id === 'string') return undefined;
            
                const { accessLevel } = collaborator.layer;
                
                if ('gitlabId' in collaborator.layer._id) {
                    const { gitlabId, ...rest } = collaborator.layer._id;
                    return { ...rest, accessLevel };  // Aquí, rest debería contener el resto de las propiedades de Layer_i
                }
                
                return undefined; // O manejar de otra manera si el gitlabId no está presente
            }).filter((layer): layer is ExtendedLayer_i => layer !== undefined);  // Usamos un type guard en el filtro para asegurarnos de remover undefined
                                                


            // ! Layers with open visibility    
            const openLayers: Layer_i[] = await Layer.find({ project: projectID, visibility: 'open' })
                                        .lean();
            
            const uniqueOpenLayersWithGuestAccess: GuestLayer_i[] = openLayers.filter((openLayer: Layer_i) =>
                openLayer._id && !layersBaseOnLevel.some(layer => layer._id && layer._id.toString() === openLayer._id.toString())
            ).map(layer => {
                if (!layer._id) return undefined; // Esto nunca debería pasar por el filtro, pero es una precaución adicional.
                const { gitlabId, __v, ...rest } = layer;
                return { ...rest, accessLevel: 'guest' } as GuestLayer_i;
            }).filter((layer): layer is GuestLayer_i => layer !== undefined); 
           ...
           // ! Combine layers
            ...
            const layers: Layer_i[] = await Layer.find({ project: projectID, visibility: { $in: levels } })
                                      .lean(); 
        
            const layersWithGuestAccess: GuestLayer_i[] = layers.map((layer: Layer_i) => {
                const { gitlabId, __v, ...rest} = layer
...
export const getCreatedLayersDates = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const layers: Layer_i[] = await Layer.find({ creator: uid, createdAt: { $gte: startDate, $lte: endDate } })
...
export const getProjectCreatedLayersDates = async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const layers: Layer_i[] = await Layer.find({ project: projectId, creator: uid, createdAt: { $gte: startDate, $lte: endDate } })
                                  .select('creator createdAt name _id project')
                                  .populate('project', 'name _id')
                                  .lean();
